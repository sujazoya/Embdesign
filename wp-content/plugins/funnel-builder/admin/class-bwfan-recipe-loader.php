...
if ( ! class_exists( 'WFFN_Recipe_Loader' ) ) {
	...
	class WFFN_Recipe_Loader {
		...
		public static function get_recipes_array( $load_new = false ) {
			$recipes_data['data']        = self::get_recipe_data( $load_new );// phpcs:ignore
			$recipes_data['total_count'] = ! empty( $recipes_data['data'] ) ? count( $recipes_data['data'] ) : 0;// phpcs:ignore
			$recipes_data['filters']     = self::get_recipes_filter();// phpcs:ignore

			return $recipes_data;// phpcs:ignore
		}
		...
		public static function get_recipe_data( $load_new ) {
			...
			$request = wp_remote_get( self::$web_url );// phpcs:ignore
			if ( is_wp_error( $request ) || wp_remote_retrieve_response_code( $request ) != 200 ) {// phpcs:ignore
				return [];
			}
